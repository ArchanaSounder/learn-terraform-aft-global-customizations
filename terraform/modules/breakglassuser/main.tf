# Data to fetch current account id
...
resource "aws_iam_user_policy_attachment" "IAMAccess" {
  ...
  policy_arn = "arn:aws:iam::aws:policy/${local.policy_name}"
}
...
# Cloudwatch event switch role capture
...
resource "aws_sns_topic_subscription" "sns-topic" {
  count     = var.sns_topic_breakglass == 1 ? 1 : 0
  topic_arn = aws_sns_topic.aws_logins.arn
  protocol  = "email"
  endpoint  = var.emailAddress[count.index]
}
